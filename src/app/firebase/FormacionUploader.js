...
export default function FormacionUploader() {
  ...
  useEffect(() => {
    ...
    const fetchTrainings = async () => {
      try {
        const trainingsSnapshot = await getDocs(collection(firestore, "trainings"));
  ...
  const handleTrainingSelection = async (trainingId) => {
    ...
    try {
      const trainingDoc = await getDoc(doc(firestore, "trainings", trainingId));
      ...
      console.error("Error loading formacion data:", error);
      setError("No se pudo cargar la formacion.");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Generate training ID - use existing ID if updating, otherwise create new one
      const trainingId = selectedTraining || doc(collection(firestore, "trainings")).id;

      let bannerUrl;
      try {
        bannerUrl = await uploadBannerImage(trainingId);
      } catch (error) {
      ...
      let flyerUrl;
      try {
        flyerUrl = await uploadFlyerImage(trainingId);
      } catch (error) {
      ...
      try {
        if (newImages.length > 0 || deletedExistingImages.length > 0) {
          galleryData = await uploadImages(trainingId);
      ...
      if (selectedTraining) {
        await updateDoc(doc(firestore, "trainings", selectedTraining), trainingData);
        ...
        setSuccess("¡Formacion actualizada con exito!");
      } else {
        // Use setDoc instead of addDoc to use the pre-generated ID
        await setDoc(doc(firestore, "trainings", trainingId), trainingData);
        setSuccess("¡Formacion creada con exito!");
      }
      ...
      setError(error.message || "No se pudo guardar la formacion.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const uploadBannerImage = async (trainingId) => {
    ...
    try {
      ...
      const bannerRef = ref(storage, `trainings/${trainingId}/images/${trainingId}_banner`);
  ...
  const uploadFlyerImage = async (trainingId) => {
    ...
    try {
      ...
      if (isVideo) {
        ...
        const flyerRef = ref(storage, `trainings/${trainingId}/flyer/${trainingId}_flyer.${fileExtension}`);
        ...
        const flyerRef = ref(storage, `trainings/${trainingId}/flyer/${trainingId}_flyer`);
...
const uploadImages = async (trainingId) => {
  ...
  const newGalleryEntries = await Promise.all(
    newImages.map(async (file, index) => {
      try {
        ...
        const imageRef = ref(storage, `trainings/${trainingId}/images/${trainingId}_gallery_${Date.now()}_${index}`);
  ...
  const addNewTraining = async () => {
    ...
    try {
      ...
      // Generate training ID before uploading images
      const trainingId = doc(collection(firestore, "trainings")).id;

      const galleryData = await uploadImages(trainingId);
      if (!galleryData) throw new Error("La carga de imagenes fallo.");

      const bannerUrl = await uploadBannerImage(trainingId);
      const flyerUrl = await uploadFlyerImage(trainingId);
      ...
      console.log("Datos de la formacion:", newTrainingData);
  

      // Use setDoc instead of addDoc to use the pre-generated ID
      await setDoc(doc(firestore, "trainings", trainingId), newTrainingData);
  
      setSuccess("¡Formacion creada con exito!");
      resetForm();
    } catch (error) {
      console.error("Error adding document:", error);
      setError("No se pudo agregar la formacion. Intentalo de nuevo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.form}>
      <div>
        <p className={styles.subtitle}>SELECCIONA UNA FORMACION PARA EDITAR</p>
        <select
          value={selectedTraining || ""}
          onChange={(e) => handleTrainingSelection(e.target.value)}
        >
          <option value="">Crear nueva formacion</option>
      ...
      {/* Formacion ID Display */}
      {selectedTraining && (
        <div className={styles.artistIdDisplay}>
          <span className={styles.artistIdLabel}>ID de la formacion:</span>
          <span className={styles.artistIdValue}>{selectedTraining}</span>
        </div>
      )}

      {/* Formacion Basic Information Container */}
      <div>
        <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.2rem', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '1px' }}>Informacion de la formacion</h3>
        <div className={styles.artistInfoContainer}>
        <div className={styles.inputGroup}>
          <p className={styles.subtitle}>TITULO DE LA FORMACION</p>
          <input
            name="name"
            placeholder="Titulo de la formacion"
            value={formData.name}
        ...
        <div className={styles.inputGroup}>
          ...
          <input
            name="subtitle"
            placeholder="Subtitulo de la formacion (opcional)"
        ...
        <div className={styles.inputGroup}>
          ...
          <textarea
            name="description"
            placeholder="Escribe la descripcion de la formacion (un parrafo por linea)"
        ...
        <div className={styles.inputGroup}>
          <p className={styles.subtitle}>FORMACION DESTACADA</p>
          <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
            ...
            />
            <span>Resaltar esta formacion en la pagina principal</span>
          </label>
        </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className={styles.artistInfoContainer}>
          ...
          <p className={styles.helpText}>Puedes agregar multiples fechas y horarios para la formacion.</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className={styles.inputGroup}>
          ...
          <input
            name="address"
            placeholder="Direccion de la formacion"
      ...
      <div>
        ...
        {/* Formacion Gallery Images */}
        <div className={styles.inputGroup}>
          <p className={styles.subtitle}>IMAGENES DE LA GALERIA DE LA FORMACION</p>
          <div
      ...
      <button type="button" onClick={handleSubmit} disabled={loading}>
        {loading ? (selectedTraining ? "Actualizando..." : "Subiendo...") : (selectedTraining ? "ACTUALIZAR FORMACION" : "AGREGAR FORMACION")}
      </button>
